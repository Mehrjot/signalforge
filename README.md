...
Resolves the goal into an audience — the AI translates intent like "win
...
Writes a personalised message per shopper — not a blast, a message
tailored to each person's name and top purchase category.
Picks the best channel per shopper — based on each customer's individual
WhatsApp / Email / SMS affinity scores, or a fixed channel if preferred.
Projects the outcome before launch — a What-If simulator estimates reach
and revenue per channel so the marketer commits with eyes open.
Runs it and reports live — the War Room streams every delivery event as
...
Full data model, lifecycle state machine, and scaling notes: `ARCHITECTURE.md`
Deployment guide (Railway + Render): `DEPLOY.md`
---
Pages & features
Page	What it does
...
Open `crm/.env` (copy from `crm/.env.example` if it doesn't exist)
Add your key:
```
GEMINI_API_KEY="your-key-here"
```
Restart the CRM (`Ctrl+C` then `npm run dev`)
...
Command Center — click any opportunity card
New Campaign — click "Build campaign", then "Launch"
War Room — watch live events stream in for ~20 seconds
Analytics — now has real revenue numbers
---
Tech stack
Layer	Choice	Why
